...
class Secure_Dacast_Session_Control {
    ...
    private function terminate_session($user_id, $session_id) {
    ...
    public function check_session_status($user_id) {
        ...
        $session_timeout = $this->heartbeat_interval * 2; // Tempo limite é 2x o intervalo do heartbeat

        if (!$last_heartbeat || (time() - $last_heartbeat) > $session_timeout) {
            $this->terminate_session($user_id, get_user_meta($user_id, 'secure_dacast_active_session', true));
            return false;
        }
